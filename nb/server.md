...
Kommandoen `sorycode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en SoryCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `sorycode.local` |
...
Sett `SORYCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `sorycode` som standard, eller sett `SORYCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `sorycode serve` og `sorycode web`.
...
Når du kjører `sorycode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk SoryCode-serveren til å samhandle med SoryCode programmatisk.
:::

Denne arkitekturen lar SoryCode støtte flere klienter og lar deg samhandle med SoryCode programmatisk.

Du kan kjøre `sorycode serve` for å starte en frittstående server. Hvis du har
SoryCode TUI kjører, vil `sorycode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av SoryCode [IDE](/docs/ide) plugins.
...
sorycode-serveren viser følgende APIer.
