...
SoryCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i SoryCode-konfigurasjonen.
...
i `~/.local/share/sorycode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i SoryCode
konfig.
...
SoryCode Zen er en liste over modeller levert av SoryCode-teamet som har vært
testet og verifisert for å fungere godt med SoryCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med SoryCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `SoryCode Zen` og gå til [sorycode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i SoryCode og er helt valgfritt å bruke.

---

## SoryCode Go

SoryCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av SoryCode-teamet som har vært
testet og verifisert for å fungere godt med SoryCode.

1. Kjør kommandoen `/connect` i TUI, velg `SoryCode Go`, og gå til [sorycode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med SoryCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører SoryCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `sorycode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i sorycode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at SoryCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom SoryCode-konfigurasjonen.
...
for å lagre token i SoryCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
SoryCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse SoryCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`sorycode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `sorycode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med SoryCode:
...
Slik bruker du Google Vertex AI med SoryCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører SoryCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom SoryCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver SoryCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver SoryCode-samtale oppført som en separat økt.
...
Du kan konfigurere SoryCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere SoryCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere SoryCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for SoryCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med SoryCode:
...
3. Kopier API-nøkkelen for bruk i SoryCode.
...
6. **Viktig**: Før du bruker skymodeller i SoryCode, må du hente modellinformasjonen lokalt:
...
SoryCode Zen er en liste over testede og verifiserte modeller levert av SoryCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>SoryCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn SoryCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom SoryCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom SoryCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med SoryCode:
...
Du kan også tilpasse modeller gjennom SoryCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med SoryCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `sorycode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar SoryCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `sorycode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk SoryCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i SoryCode-konfigurasjonen.
