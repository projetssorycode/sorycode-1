Du kan legge til eksterne verktøy i SoryCode ved å bruke _Model Context Protocol_ eller MCP. SoryCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [SoryCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/sorycode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
SoryCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil SoryCode:
...
Hvis serveren krever autentisering, vil SoryCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `sorycode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil SoryCode lagre tokens sikkert i `~/.local/share/sorycode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i SoryCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom SoryCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble SoryCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver SoryCode-samtale oppført som en separat økt.
