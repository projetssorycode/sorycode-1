:::caution
Moduser konfigureres nå gjennom alternativet `agent` i SoryCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i SoryCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom SoryCode-konfigurasjonen.
...
SoryCode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.sorycode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `sorycode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/sorycode/modes/`
- Prosjekt: `.sorycode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker SoryCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale SoryCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.sorycode/modes/` for prosjektspesifikke moduser eller `~/.config/sorycode/modes/` for globale moduser:
