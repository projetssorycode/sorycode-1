...
Det er to typer agenter i SoryCode; primære agenter og underagenter.
...
SoryCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
SoryCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

SoryCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `sorycode.json`:
...
- Globalt: `~/.config/sorycode/agents/`
- Per prosjekt: `.sorycode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker SoryCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale SoryCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i SoryCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [SoryCode Zen](/docs/zen), vil du bruke `sorycode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `sorycode models` for å se en liste over tilgjengelige modeller.
:::
