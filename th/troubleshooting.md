หากต้องการแก้ไขข้อบกพร่องเกี่ยวกับ SoryCode ให้เริ่มต้นด้วยการตรวจสอบบันทึกและข้อมูลในเครื่องที่จัดเก็บไว้ในดิสก์
...
- **Windows**: กด `WIN+R` แล้ววาง `%USERPROFILE%\.local\share\sorycode\log`

ไฟล์บันทึกจะถูกตั้งชื่อด้วยการประทับเวลา (เช่น `2025-01-09T123456.log`) และไฟล์บันทึกล่าสุด 10 ไฟล์จะถูกเก็บไว้

คุณสามารถตั้งค่าระดับการบันทึกด้วยตัวเลือกบรรทัดคำสั่ง `--log-level` เพื่อรับข้อมูลการแก้ไขข้อบกพร่องโดยละเอียดเพิ่มเติม ตัวอย่างเช่น `sorycode --log-level DEBUG`

---

## พื้นที่จัดเก็บ

sorycode เก็บข้อมูลเซสชันและข้อมูลแอปพลิเคชันอื่น ๆ ไว้บนดิสก์ที่:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: กด `WIN+R` แล้ววาง `%USERPROFILE%\.local\share\sorycode`
...
SoryCode Desktop รันเซิร์ฟเวอร์ SoryCode ในเครื่อง (ไฟล์ `sorycode-cli` sidecar) ในเบื้องหลัง ปัญหาส่วนใหญ่มีสาเหตุมาจากปลั๊กอินที่ทำงานผิดปกติ แคชเสียหาย หรือการตั้งค่าเซิร์ฟเวอร์ไม่ถูกต้อง

### การตรวจสอบอย่างรวดเร็ว

- ออกจากระบบโดยสมบูรณ์แล้วเปิดแอปใหม่อีกครั้ง
- หากแอปแสดงหน้าจอข้อผิดพลาด ให้คลิก **รีสตาร์ท** และคัดลอกรายละเอียดข้อผิดพลาด
- macOS เท่านั้น: เมนู `SoryCode` -> **โหลด Webview ใหม่** (ช่วยได้หาก UI ว่างเปล่า/frozen)
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (หรือ `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (การติดตั้งเก่ากว่า): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: กด `WIN+R` แล้ววาง `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
SoryCode ยังสามารถโหลดปลั๊กอินในเครื่องจากดิสก์ได้ ย้ายสิ่งเหล่านี้ออกไปชั่วคราว (หรือเปลี่ยนชื่อโฟลเดอร์) และรีสตาร์ทแอปเดสก์ท็อป:

- **ปลั๊กอินสากล**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: กด `WIN+R` แล้ววาง `%USERPROFILE%\.config\sorycode\plugins`
...
หากการปิดใช้งานปลั๊กอินไม่ได้ผล (หรือการติดตั้งปลั๊กอินค้าง) ให้ล้างแคชเพื่อให้ SoryCode สามารถสร้างใหม่ได้

1. ออกจาก SoryCode Desktop โดยสมบูรณ์
2. ลบไดเรกทอรีแคช:

- **macOS**: ตัวค้นหา -> `Cmd+Shift+G` -> วาง `~/.cache/sorycode`
- **Linux**: ลบ `~/.cache/sorycode` (หรือรัน `rm -rf ~/.cache/sorycode`)
- **Windows**: กด `WIN+R` แล้ววาง `%USERPROFILE%\.cache\sorycode`

3. รีสตาร์ทเดสก์ท็อป SoryCode

---

### แก้ไขปัญหาการเชื่อมต่อเซิร์ฟเวอร์

SoryCode Desktop สามารถเริ่มต้นเซิร์ฟเวอร์ภายในเครื่องของตนเองได้ (ค่าเริ่มต้น) หรือเชื่อมต่อกับเซิร์ฟเวอร์ URL ที่คุณกำหนดค่าไว้
...
หาก `sorycode.json(c)` ของคุณมีส่วน `server` ให้ลบออกชั่วคราวแล้วรีสตาร์ทแอปเดสก์ท็อป

#### ตรวจสอบตัวแปรสภาพแวดล้อม

หากคุณตั้งค่า `SORYCODE_PORT` ในสภาพแวดล้อมของคุณ แอปเดสก์ท็อปจะพยายามใช้พอร์ตนั้นสำหรับเซิร์ฟเวอร์ภายในเครื่อง

- ยกเลิกการตั้งค่า `SORYCODE_PORT` (หรือเลือกพอร์ตว่าง) แล้วรีสตาร์ท
...
บน Windows SoryCode Desktop ต้องใช้ Microsoft Edge **WebView2 Runtime** หากแอปเปิดเป็นหน้าต่างว่างหรือไม่เริ่มทำงาน ให้ติดตั้ง/update WebView2 แล้วลองอีกครั้ง

---

### Windows: ปัญหาด้านประสิทธิภาพทั่วไป

หากคุณประสบปัญหาประสิทธิภาพการทำงานช้า ปัญหาการเข้าถึงไฟล์ หรือปัญหา terminal บน Windows ให้ลองใช้ [WSL (ระบบย่อย Windows สำหรับ Linux)](/docs/windows-wsl) WSL มอบสภาพแวดล้อม Linux ที่ทำงานร่วมกับคุณสมบัติของ SoryCode ได้อย่างราบรื่นยิ่งขึ้น

---

### การแจ้งเตือนไม่แสดง

SoryCode Desktop จะแสดงการแจ้งเตือนของระบบเฉพาะเมื่อ:

- การแจ้งเตือนเปิดใช้งานสำหรับ SoryCode ในการตั้งค่าระบบปฏิบัติการของคุณและ
...
1. ออกจากเดสก์ท็อป SoryCode
2. ค้นหาและลบไฟล์เหล่านี้ (อยู่ในไดเร็กทอรีข้อมูลแอป SoryCode Desktop):

- `sorycode.settings.dat` (เซิร์ฟเวอร์เริ่มต้นของเดสก์ท็อป URL)
- `sorycode.global.dat` และ `sorycode.workspace.*.dat` (สถานะ UI เช่น เซิร์ฟเวอร์ล่าสุด/projects)
...
หากต้องการทราบว่าคุณมีสิทธิ์เข้าถึงรุ่นใด ให้เรียกใช้ `sorycode models`
...
2. หากปัญหายังคงอยู่ ให้ลองล้างการกำหนดค่าที่เก็บไว้:
   ...
   บน Windows กด `WIN+R` และลบ: `%USERPROFILE%\.local\share\sorycode`
...
หากคุณพบข้อผิดพลาดในการโทร API อาจเนื่องมาจากแพ็คเกจผู้ให้บริการที่ล้าสมัย sorycode จะติดตั้งแพ็คเกจของผู้ให้บริการแบบไดนามิก (OpenAI, Anthropic, Google ฯลฯ) ตามความจำเป็น และแคชไว้ในเครื่อง
...
1. ล้างแคชแพ็คเกจผู้ให้บริการ:
   ...
   บน Windows กด `WIN+R` และลบ: `%USERPROFILE%\.cache\sorycode`

2. รีสตาร์ท sorycode เพื่อติดตั้งแพ็คเกจผู้ให้บริการล่าสุดอีกครั้ง

การดำเนินการนี้จะบังคับให้ sorycode ดาวน์โหลดแพ็คเกจผู้ให้บริการเวอร์ชันล่าสุด ซึ่งมักจะแก้ไขปัญหาความเข้ากันได้กับพารามิเตอร์โมเดลและการเปลี่ยนแปลง API
...
sorycode จะตรวจพบว่าคุณใช้ Wayland และชอบ `wl-clipboard` หรือไม่เช่นนั้นจะพยายามค้นหาเครื่องมือคลิปบอร์ดตามลำดับ: `xclip` และ `xsel`