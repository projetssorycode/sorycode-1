For at fejlfinde problemer med SoryCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\sorycode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `sorycode --log-level DEBUG`.

---

## Lagring

sorycode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\sorycode`
...
SoryCode Desktop kører en lokal SoryCode-server (`sorycode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `SoryCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (eller `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
SoryCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\sorycode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så SoryCode kan genopbygge den.

1. Afslut SoryCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/sorycode`
- **Linux**: slet `~/.cache/sorycode` (eller kør `rm -rf ~/.cache/sorycode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\sorycode`

3. Start SoryCode Desktop på ny.

---

### Løs problemer med serverforbindelse

SoryCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `sorycode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `SORYCODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `SORYCODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver SoryCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i SoryCode.

---

### Meddelelser vises ikke

SoryCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for SoryCode i dine OS-indstillinger, og
...
1. Afslut SoryCode Desktop.
2. Find og slet disse filer (de findes i SoryCode Desktop-appens datamappe):

- `sorycode.settings.dat` (skrivebordsstandardserver URL)
- `sorycode.global.dat` og `sorycode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med SoryCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `sorycode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\sorycode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. sorycode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\sorycode`

2. Start sorycode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge sorycode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
sorycode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.