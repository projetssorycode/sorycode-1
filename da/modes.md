:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i sorycode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i sorycode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem sorycode-konfigurationen.
...
sorycode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.sorycode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `sorycode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger sorycode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale sorycode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.sorycode/modes/` for projektspecifikke tilstande eller `~/.config/sorycode/modes/` for globale tilstande:
