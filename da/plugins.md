Plugins giver dig mulighed for at udvide SoryCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre SoryCodes standardadfærd.
...
- `.sorycode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/sorycode/node_modules/`.
...
1. Global konfiguration (`~/.config/sorycode/sorycode.json`)
2. Projektkonfiguration (`sorycode.json`)
3. Global plugin-mappe (`~/.config/sorycode/plugins/`)
4. Projekt plugin bibliotek (`.sorycode/plugins/`)
...
SoryCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En sorycode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide sorycode.
...
:::note
Hvis du bruger SoryCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå sorycode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til sorycode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som sorycode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for sorycode sammen med indbyggede værktøjer.
