...
Kommandoen `sorycode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en sorycode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `sorycode.local` |
...
Indstil `SORYCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `sorycode`, eller indstil `SORYCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `sorycode serve` og `sorycode web`.
...
Når du kører `sorycode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug sorycode-serveren til at interagere med sorycode programmatisk.
:::

Denne arkitektur lader sorycode understøtte flere klienter og lader dig interagere med sorycode programmatisk.

Du kan køre `sorycode serve` for at starte en selvstændig server. Hvis du har
sorycode TUI kørende, vil `sorycode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af SoryCode [IDE](/docs/ide) plugins.
...
SoryCode-serveren viser følgende API'er.
