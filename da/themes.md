Med SoryCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger SoryCode vårt eget `sorycode`-tema.
...
SoryCode kommer med flere innebygde temaer.
...
- Vil at SoryCode skal matche terminalens utseende
...
SoryCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/sorycode/themes/*.json` eller `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.sorycode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.sorycode/themes/*.json`
