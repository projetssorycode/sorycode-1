...
SoryCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i SoryCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/sorycode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i SoryCode-konfigurationen.
...
SoryCode Zen er en liste over modeller leveret af SoryCode-teamet, som er blevet testet og verificeret til at fungere godt med SoryCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med SoryCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg sorycode og gå til [sorycode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i SoryCode og er helt valgfrit at bruge.

---

## SoryCode Go

SoryCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af SoryCode-teamet, som er testet og verificeret til at fungere godt med SoryCode.

1. Kør kommandoen `/connect` i TUI, vælg `SoryCode Go`, og gå til [sorycode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med SoryCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører sorycode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `sorycode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i SoryCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at sorycode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem sorycode-konfigurationen.
...
gemme tokenet i sorycode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
SoryCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse SoryCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`sorycode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `sorycode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med SoryCode:
...
Sådan bruger du Google Vertex AI med SoryCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører sorycode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem sorycode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver SoryCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver SoryCode-samtale opført som en separat session.
...
Du kan konfigurere sorycode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere sorycode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere sorycode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for SoryCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med SoryCode:
...
3. Kopier API-nøglen til brug i SoryCode.
...
6. **Vigtigt**: Før du bruger skymodeller i SoryCode, skal du hente modelinformationen lokalt:
...
SoryCode Zen er en liste over testede og verificerede modeller leveret af SoryCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>SoryCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Indtast SoryCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem sorycode-konfigurationen.
...
5. Du kan også tilpasse dem gennem sorycode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med SoryCode:
...
Du kan også tilpasse modeller gennem sorycode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med sorycode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `sorycode.json`-filen i projektkataloget:
...
`limit`-felterne lader SoryCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `sorycode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek sorycode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i sorycode-konfigurationen.
