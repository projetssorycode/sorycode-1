...
The `sorycode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an sorycode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `sorycode.local` |
...
Set `SORYCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `sorycode`, or set `SORYCODE_SERVER_USERNAME` to override it. This applies to both `sorycode serve` and `sorycode web`.
...
When you run `sorycode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the sorycode server to interact with sorycode programmatically.
:::

This architecture lets sorycode support multiple clients and allows you to interact with sorycode programmatically.

You can run `sorycode serve` to start a standalone server. If you have the
sorycode TUI running, `sorycode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the SoryCode [IDE](/docs/ide) plugins.
...
The sorycode server exposes the following APIs.
