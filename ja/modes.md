:::caution
モードは、SoryCode 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、SoryCode 設定を通じて独自の設定を行います。
...
SoryCode には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.sorycode/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`sorycode.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/sorycode/modes/`
- プロジェクト: `.sorycode/modes/`
...
温度が指定されていない場合、SoryCode はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.sorycode/modes/` に、グローバルモードの場合は `~/.config/sorycode/modes/` に作成します。
