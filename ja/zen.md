...
SoryCode Zen は、SoryCode チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
SoryCode Zen は現在ベータ版です。
:::
Zen は SoryCode の他のプロバイダーと同様に機能します。 SoryCode Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、SoryCode を使用するために使用する必要はありません。
...
:::tip
SoryCode で適切に動作する、厳選されたモデルとプロバイダーのグループをテストしました。
:::
...
SoryCode Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

SoryCode Zen は、SoryCode の他のプロバイダーと同様に機能します。

1. **<a href={console}>SoryCode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、SoryCode Zen を選択して API キーを貼り付けます。
...
SoryCode 設定の [model id](/docs/config/#models)
`sorycode/<model-id>` 形式を使用します。たとえば、GPT 5.2 Codex の場合は、
設定で `sorycode/gpt-5.2-codex` を使用してください。
...
- MiniMax M2.5 Free は期間限定で SoryCode で利用可能です。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で SoryCode で無料で利用できるステルスモデルです。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
...
私たちは次の目的で SoryCode Zen を作成しました。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に SoryCode で他のプロバイダーも使用できるようにします。