SoryCode の共有機能を使用すると、SoryCode の会話への公開リンクを作成できるため、チームメイトと共同作業したり、他の人から助けを得ることができます。
...
SoryCode は、会話の共有方法を制御する 3 つの共有モードをサポートしています。

---

### 手動（デフォルト）

デフォルトでは、SoryCode は手動共有モードを使用します。セッションは自動的には共有されませんが、`/share` コマンドを使用して手動で共有できます。
...
これを特定のプロジェクトのチーム全体に強制するには、それをプロジェクトの `sorycode.json` に追加し、Git にチェックインします。
...
[組織内での SoryCode の使用について詳しくは、](/docs/enterprise) をご覧ください。