外掛允許您透過掛鉤各種事件和自訂行為來擴展 SoryCode。您可以建立外掛來新增新功能、整合外部服務，或修改 SoryCode 的預設行為。
...
- `.sorycode/plugins/` - 專案級外掛
- `~/.config/sorycode/plugins/` - 全域外掛
...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/sorycode/node_modules/` 中。
...
1. 全域設定 (`~/.config/sorycode/sorycode.json`)
2. 專案設定 (`sorycode.json`)
3. 全域外掛目錄 (`~/.config/sorycode/plugins/`)
4. 專案外掛目錄 (`.sorycode/plugins/`)
...
SoryCode 會在啟動時執行 `bun install` 來安裝這些相依套件。之後您的外掛和工具就可以匯入它們了。
...
- `directory`：當前工作目錄。
- `worktree`：git 工作樹路徑。
- `client`：用於與 AI 互動的 SoryCode SDK 用戶端。
...
:::note
如果您使用 SoryCode 桌面應用程式，它可以在回應就緒或工作階段出錯時自動傳送系統通知。
:::
...
`tool` 輔助函式用於建立 SoryCode 可呼叫的自訂工具。它接受一個 Zod schema 函式，並回傳一個工具定義，包含：

- `description`：工具的功能描述
- `args`：工具參數的 Zod schema
- `execute`：工具被呼叫時執行的函式

您的自訂工具將與內建工具一起在 SoryCode 中可用。
