SoryCode 透過 GitLab CI/CD 管線或 GitLab Duo 與您的 GitLab 工作流程整合。

在這兩種情況下，SoryCode 都將在您的 GitLab Runner 上執行。

---

## GitLab CI

SoryCode 可以在常規的 GitLab 管線中執行。您可以將其作為 [CI 元件](https://docs.gitlab.com/ee/ci/components/) 整合到管線中。

這裡我們使用的是社群建立的 SoryCode CI/CD 元件 — [nagyv/gitlab-sorycode](https://gitlab.com/nagyv/gitlab-opencode)。

---

### 功能特性

- **按任務自訂設定**：使用自訂設定目錄來設定 SoryCode，例如 `./config/#custom-directory`，以便為每次 SoryCode 呼叫啟用或停用特定功能。
- **最小化設定**：CI 元件會在背景完成 SoryCode 的設定，您只需建立 SoryCode 設定和初始提示詞即可。
...
1. 將您的 SoryCode 身分驗證 JSON 作為檔案類型的 CI 環境變數儲存在 **Settings** > **CI/CD** > **Variables** 下。請確保將其標記為「Masked and hidden」。
...
SoryCode 與您的 GitLab 工作流程整合。
在評論中提及 `@sorycode`，SoryCode 將在您的 GitLab CI 管線中執行任務。

---

### 功能特性

- **Issue 分類**：讓 SoryCode 調查某個 Issue 並為您解釋。
- **修復與實作**：讓 SoryCode 修復 Issue 或實作某個功能。它會建立一個新分支，並提交包含變更的合併請求。
- **安全可靠**：SoryCode 在您的 GitLab Runner 上執行。

---

### 設定

SoryCode 在您的 GitLab CI/CD 管線中執行，以下是設定所需的步驟：
...
以下是在 GitLab 中使用 SoryCode 的一些範例。

:::tip
您可以設定使用不同於 `@sorycode` 的觸發詞。
:::
...
- **修復 Issue**
  ...
  SoryCode 會建立一個新分支，實作變更，並提交包含變更的合併請求。
