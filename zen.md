...
SoryCode Zen is a list of tested and verified models provided by the SoryCode team.

:::note
SoryCode Zen is currently in beta.
:::

Zen works like any other provider in SoryCode. You login to SoryCode Zen and get
...
:::tip
We tested a select group of models and providers that work well with SoryCode.
:::
...
SoryCode Zen is an AI gateway that gives you access to these models.

---

## How it works

SoryCode Zen works like any other provider in SoryCode.

1. You sign in to **<a href={console}>SoryCode Zen</a>**, add your billing
   details, and copy your API key.
2. You run the `/connect` command in the TUI, select SoryCode Zen, and paste your API key.
...
The [model id](/docs/config/#models) in your SoryCode config
uses the format `sorycode/<model-id>`. For example, for GPT 5.3 Codex, you would
use `sorycode/gpt-5.3-codex` in your config.
...
- MiniMax M2.5 Free is available on SoryCode for a limited time. The team is using this time to collect feedback and improve the model.
- Big Pickle is a stealth model that's free on SoryCode for a limited time. The team is using this time to collect feedback and improve the model.
...
4. Have **no lock-in** by allowing you to use it with any other coding agent. And always let you use any other provider with SoryCode as well.