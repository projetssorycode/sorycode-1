With SoryCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, SoryCode uses our own `sorycode` theme.
...
SoryCode comes with several built-in themes.
...
- Want SoryCode to match their terminal's appearance
...
SoryCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/sorycode/themes/*.json` or `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.sorycode/themes/*.json`
4. **Current working directory** - Defined in `./.sorycode/themes/*.json`
