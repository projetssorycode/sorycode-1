...
sorycode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji sorycode.
...
w `~/.local/share/sorycode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim sorycode
config.
...
SoryCode Zen to lista modeli dostarczonych przez zespół sorycode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z sorycode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od SoryCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `SoryCode Zen` i przejdź do [sorycode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w sorycode i jest całkowicie opcjonalny w użyciu.

---

## SoryCode Go

SoryCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół sorycode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z sorycode.

1. Uruchom polecenie `/connect` w TUI, wybierz `SoryCode Go` i przejdź do [sorycode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z sorycode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania sorycode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `sorycode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w sorycode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod sorycode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji sorycode.
...
sorycode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować sorycode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`sorycode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `sorycode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem sorycode:
...
Aby używać Google Vertex AI z sorycode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania sorycode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą sorycode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację sorycode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja sorycode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować sorycode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować sorycode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować sorycode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem sorycode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z sorycode:
...
3. Skopiuj klucz API do użycia w sorycode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w sorycode musisz pobrać informacje o modelu lokalnie:
...
SoryCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół sorycode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>SoryCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API sorycode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji sorycode.
...
5. Możesz je także dostosować za pomocą konfiguracji sorycode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z sorycode:
...
Możesz także dostosować modele za pomocą konfiguracji sorycode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z sorycode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `sorycode.json` w katalogu projektu:
...
Pola `limit` pozwalają sorycode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `sorycode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację sorycode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji sorycode.
