Możesz dodać dodatkowe informacje do sorycode za pomocą _Model Context Protocol_, w skrócie MCP. sorycode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [sorycode Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/sorycode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
sorycode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, sorycode:
...
Jeśli serwer wymaga uwierzytelnienia, sorycode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `sorycode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji sorycode bezpieczne przechowa tokeny w `~/.local/share/sorycode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w sorycode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie sorycode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć sorycode z kontem Sentry.
