...
Komenda `sorycode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient sorycode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `sorycode.local` |
...
Ustaw `SORYCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `sorycode` lub ustaw `SORYCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `sorycode serve`, jak i `sorycode web`.
...
Kiedy uruchomisz `sorycode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera sorycode do programowej interakcji z sorycode.
:::

Ta architektura umożliwia obsługę wielu klientów przez sorycode i programową interakcję z kodem otwartym.

Możesz uruchomić `sorycode serve`, aby uruchomić samodzielny serwer. Jeśli masz
sorycode TUI działa, `sorycode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki sorycode [IDE](/docs/ide).
...
Serwer sorycode udostępnia następujące interfejsy API.
