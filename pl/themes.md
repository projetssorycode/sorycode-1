Dzięki sorycode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie sorycode używa naszego własnego motywu `sorycode`.
...
sorycode ma kilka wbudowanych motywów.
...
- Chcą, aby SoryCode pasował do wyglądu ich terminala
...
sorycode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/sorycode/themes/*.json` lub `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.sorycode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.sorycode/themes/*.json`
