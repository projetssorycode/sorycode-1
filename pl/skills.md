Umiejętności agenta pozwalają sorycode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
sorycode przeszukuje te lokalizacje:

- Project config: `.sorycode/skills/<name>/SKILL.md`
- Global config: `~/.config/sorycode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu sorycode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.sorycode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/sorycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.sorycode/skills/git-release/SKILL.md` w ten sposób:
...
sorycode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `sorycode.json`:
...
**Dla agentów wbudowanych** (w `sorycode.json`):
