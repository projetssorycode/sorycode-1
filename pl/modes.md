:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji sorycode.
...
te lub skonfiguruj własne za pomocą konfiguracji sorycode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.sorycode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `sorycode.json`:
...
- Globalnie: `~/.config/sorycode/modes/`
...
Jeśli nie ma wpływu na temperaturę, sorycode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna sorycode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.sorycode/modes/` dla trybów zapisanych dla projektu lub `~/.config/sorycode/modes/` dla trybów globalnych:
