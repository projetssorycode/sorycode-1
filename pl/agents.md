...
W SoryCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
SoryCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
SoryCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

SoryCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `sorycode.json`:
...
- Globalnie: `~/.config/sorycode/agents/`
- W projekcie: `.sorycode/agents/`
...
Jeśli nie zostanie skonfigurowana, SoryCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji SoryCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji SoryCode w `provider/model-id`. Na przykład, [SoryCode Zen](/docs/zen), użyj `sorycode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `sorycode models`, aby zobaczyć dostępne modele.
:::
