Aby debugować problemy z sorycode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\sorycode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `sorycode --log-level DEBUG`.

---

## Przechowywanie danych

sorycode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\sorycode`
...
sorycode Desktop uruchamia w tle lokalny serwer sorycode (sidecar `sorycode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `SoryCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (lub `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
sorycode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\sorycode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby sorycode mógł ją odbudować.

1. Całkowicie zamknij sorycode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/sorycode`
- **Linux**: usuń `~/.cache/sorycode` (lub uruchom `rm -rf ~/.cache/sorycode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\sorycode`

3. Uruchom ponownie sorycode Desktop.

---

### Napraw problemy z połączeniem z serwerem

sorycode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `sorycode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `SORYCODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `SORYCODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows sorycode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami sorycode.

---

### Brak powiadomień

sorycode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla sorycode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie sorycode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji sorycode Desktop):

- `sorycode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `sorycode.global.dat` i `sorycode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z sorycode:
...
3. Upewnij się, że masz najnowszą wersję z `sorycode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `sorycode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\sorycode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. sorycode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\sorycode`

2. Uruchom ponownie kod sorycode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to sorycode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
sorycode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.