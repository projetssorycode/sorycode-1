Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. SoryCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w SoryCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (lub `SORYCODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy SoryCode lub gdy zmienna środowiskowa `SORYCODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu SoryCode:
