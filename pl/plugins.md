Wtyczki umożliwiające wyłączenie sorycode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania sorycode.
...
- `.sorycode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/sorycode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/sorycode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/sorycode/sorycode.json`)
2. Konfiguracja projektu (`sorycode.json`)
3. Globalny katalog wtyczek (`~/.config/sorycode/plugins/`)
4. Katalog wtyczek projektu (`.sorycode/plugins/`)
...
sorycode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z sorycode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do sorycode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej sorycode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj sorycode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do sorycode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod sorycode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
