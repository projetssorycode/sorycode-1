...
O comando `sorycode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente sorycode pode usar.
...
```bash
sorycode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `sorycode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
sorycode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `SORYCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `sorycode`, ou defina `SORYCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `sorycode serve` quanto ao `sorycode web`.

```bash
SORYCODE_SERVER_PASSWORD=your-password sorycode serve
```
...
Quando você executa `sorycode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor sorycode para interagir com o sorycode programaticamente.
:::

Essa arquitetura permite que o sorycode suporte múltiplos clientes e permite que você interaja com o sorycode programaticamente.

Você pode executar `sorycode serve` para iniciar um servidor autônomo. Se você tiver o TUI do sorycode em execução, `sorycode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do sorycode [IDE](/docs/ide).
...
O servidor sorycode expõe as seguintes APIs.
