...
Existem dois tipos de agentes no sorycode; agentes primários e subagentes.
...
sorycode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
sorycode vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

sorycode vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `sorycode.json`:
...
Se nenhuma temperatura for especificada, o sorycode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do sorycode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do sorycode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [SoryCode Zen](/docs/zen), você usaria `sorycode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `sorycode models` para ver uma lista dos modelos disponíveis.
:::
