Plugins permitem que você estenda o sorycode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do sorycode.
...
- `.sorycode/plugins/` - Plugins em nível de projeto
- `~/.config/sorycode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/sorycode/node_modules/`.
...
1. Configuração global (`~/.config/sorycode/sorycode.json`)
2. Configuração do projeto (`sorycode.json`)
3. Diretório de plugins global (`~/.config/sorycode/plugins/`)
4. Diretório de plugins do projeto (`.sorycode/plugins/`)
...
O sorycode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do sorycode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o sorycode.
...
:::note
Se você estiver usando o aplicativo desktop sorycode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o sorycode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao sorycode:
...
O helper `tool` cria uma ferramenta personalizada que o sorycode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o sorycode junto com as ferramentas integradas.
