...
O SoryCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do SoryCode.

:::note
O SoryCode Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no SoryCode. Você faz login no SoryCode Zen e obtém sua chave de API. É **completamente opcional** e você não precisa usá-lo para utilizar o SoryCode.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o SoryCode.
:::
...
O SoryCode Zen é um gateway de IA que lhe dá acesso a esses modelos.

---

## Como funciona

O SoryCode Zen funciona como qualquer outro provedor no SoryCode.

1. Você faz login no **<a href={console}>SoryCode Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona SoryCode Zen e cola sua chave de API.
...
O [id do modelo](/docs/config/#models) na sua configuração do SoryCode usa o formato `sorycode/<model-id>`. Por exemplo, para GPT 5.2 Codex, você usaria `sorycode/gpt-5.2-codex` na sua configuração.
...
- MiniMax M2.5 Free está disponível no SoryCode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo oculto que está gratuito no SoryCode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
...
4. Não ter **vinculação** permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o SoryCode também.