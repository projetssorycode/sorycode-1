As ferramentas permitem que o LLM execute ações em sua base de código. O sorycode vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no sorycode.
...
:::note
Esta ferramenta está disponível apenas quando `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `SORYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor sorycode ou quando a variável de ambiente `SORYCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o sorycode:
