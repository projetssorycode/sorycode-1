O sorycode se integra ao seu fluxo de trabalho do GitLab através do seu pipeline CI/CD do GitLab ou com o GitLab Duo.

Em ambos os casos, o sorycode será executado em seus runners do GitLab.

---

## GitLab CI

O sorycode funciona em um pipeline regular do GitLab. Você pode integrá-lo a um pipeline como um [componente CI](https://docs.gitlab.com/ee/ci/components/)

Aqui estamos usando um componente CI/CD criado pela comunidade para sorycode — [nagyv/gitlab-sorycode](https://gitlab.com/nagyv/gitlab-opencode).

---

### Recursos

- **Use configuração personalizada por trabalho**: Configure o sorycode com um diretório de configuração personalizado, por exemplo `./config/#custom-directory` para habilitar ou desabilitar funcionalidades por invocação do sorycode.
- **Configuração mínima**: O componente CI configura o sorycode em segundo plano, você só precisa criar a configuração do sorycode e o prompt inicial.
...
1. Armazene seu JSON de autenticação do sorycode como variáveis de ambiente do tipo File em **Configurações** > **CI/CD** > **Variáveis**. Certifique-se de marcá-las como "Mascaradas e ocultas".
   ...
   include:
     - component: $CI_SERVER_FQDN/nagyv/gitlab-sorycode/sorycode@2
       inputs:
         ...
         auth_json: $SORYCODE_AUTH_JSON # O nome da variável para seu JSON de autenticação do SoryCode
...
O sorycode se integra ao seu fluxo de trabalho do GitLab.
Mencione `@sorycode` em um comentário, e o sorycode executará tarefas dentro do seu pipeline CI do GitLab.
...
- **Corrigir e implementar**: Peça ao sorycode para corrigir um problema ou implementar uma funcionalidade.
  Ele criará um novo branch e abrirá uma solicitação de mesclagem com as alterações.
- **Seguro**: O sorycode é executado em seus runners do GitLab.

---

### Configuração

O sorycode é executado em seu pipeline CI/CD do GitLab, aqui está o que você precisará para configurá-lo:
...
6.  Crie um arquivo de configuração de fluxo, aqui está um exemplo:
    ...
    commands:
      ...
      - echo $GITLAB_HOST
      - echo "Criando configuração de auth do SoryCode"
...
Aqui estão alguns exemplos de como você pode usar o sorycode no GitLab.

:::tip
Você pode configurar para usar uma frase de gatilho diferente de `@sorycode`.
:::
...
- **Explicar um problema**
  ...
  O sorycode lerá o problema e responderá com uma explicação clara.
...
- **Corrigir um problema**
  ...
  O sorycode criará um novo branch, implementará as alterações e abrirá uma solicitação de mesclagem com as alterações.
...
- **Revisar solicitações de mesclagem**
  ...
  O sorycode revisará a solicitação de mesclagem e fornecerá feedback.