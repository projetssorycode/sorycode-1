Com o sorycode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o sorycode usa nosso próprio tema `sorycode`.
...
O sorycode vem com vários temas embutidos.
...
- Querem que o sorycode corresponda à aparência do seu terminal
...
O sorycode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/sorycode/themes/*.json` ou `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.sorycode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.sorycode/themes/*.json`
