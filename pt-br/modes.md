:::caution
Os modos agora são configurados através da opção `agent` na configuração do sorycode. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no sorycode permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do sorycode.
...
O sorycode vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.sorycode/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `sorycode.json`:
...
- Global: `~/.config/sorycode/modes/`
- Projeto: `.sorycode/modes/`
...
Se nenhuma temperatura for especificada, o sorycode usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do sorycode quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.sorycode/modes/` para modos específicos do projeto ou `~/.config/sorycode/modes/` para modos globais:
