As habilidades do agente permitem que o sorycode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O sorycode pesquisa nesses locais:

- Configuração do projeto: `.sorycode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/sorycode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o sorycode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.sorycode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/sorycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.sorycode/skills/git-release/SKILL.md` assim:
...
O sorycode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `sorycode.json`:
...
**Para agentes embutidos** (em `sorycode.json`):
