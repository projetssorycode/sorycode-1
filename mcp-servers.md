You can add external tools to SoryCode using the _Model Context Protocol_, or MCP. SoryCode supports both local and remote servers.
...
You can define MCP servers in your [SoryCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/sorycode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
SoryCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, SoryCode will:
...
If the server requires authentication, SoryCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `sorycode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, SoryCode will store the tokens securely in `~/.local/share/sorycode/mcp-auth.json`.
...
Your MCPs are available as tools in SoryCode, alongside built-in tools. So you can manage them through the SoryCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect SoryCode to your Sentry account.
