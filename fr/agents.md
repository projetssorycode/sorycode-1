...
Il existe deux types d'agents dans SoryCode ; agents primaires et sous-agents.
...
SoryCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
SoryCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

SoryCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `sorycode.json` :
...
- Global : `~/.config/sorycode/agents/`
- Par projet : `.sorycode/agents/`
...
Si aucune température n'est spécifiée, SoryCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale SoryCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration SoryCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [SoryCode Zen](/docs/zen), vous utiliserez `sorycode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="sorycode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="sorycode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `sorycode models` pour voir une liste des modèles disponibles.
:::
...
```markdown title="~/.config/sorycode/agents/docs-writer.md"
---
...
```markdown title="~/.config/sorycode/agents/security-auditor.md"
---
