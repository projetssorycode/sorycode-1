...
SoryCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration SoryCode.
...
en `~/.local/share/sorycode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre SoryCode
configuration.
...
SoryCode Zen est une liste de modèles fournis par l'équipe SoryCode qui ont été
testé et vérifié pour fonctionner correctement avec SoryCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par SoryCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez sorycode et dirigez-vous vers [sorycode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans SoryCode et son utilisation est totalement facultative.

---

## SoryCode Go

SoryCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe SoryCode qui ont été
testé et vérifié pour fonctionner correctement avec SoryCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `SoryCode Go` et rendez-vous sur [sorycode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec SoryCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de sorycode :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY sorycode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile sorycode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX sorycode
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `sorycode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans SoryCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que sorycode fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX sorycode
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX sorycode
   ```
...
Vous pouvez également ajouter des modèles via votre configuration sorycode.
...
pour stocker le jeton dans le stockage d'authentification sorycode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
SoryCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller SoryCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `sorycode.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuration

Personnalisez via `sorycode.json` :
...
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["@gitlab/sorycode-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec sorycode :
...
Pour utiliser Google Vertex AI avec SoryCode :
...
Définissez-les lors de l'exécution de sorycode.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id sorycode
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via sorycode, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/sorycode/sorycode.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation SoryCode en tant que session dans Helicone.

```bash
npm install -g sorycode-helicone-session
```
...
```json title="sorycode.json"
{
  "plugin": ["sorycode-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation SoryCode répertoriée comme une session distincte.
...
Vous pouvez configurer sorycode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="sorycode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer sorycode pour utiliser des modèles locaux via LM Studio.

```json title="sorycode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer sorycode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour SoryCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::

```json title="sorycode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec SoryCode :
...
3. Copiez la clé API à utiliser dans SoryCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans SoryCode, vous devez extraire les informations du modèle localement :
...
SoryCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe SoryCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>SoryCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé SoryCode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration sorycode.
...
5. Vous pouvez également les personnaliser via votre configuration sorycode. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' sorycode
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group sorycode
   ```
...
Vous pouvez également personnaliser les modèles via votre configuration sorycode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec sorycode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in sorycode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `sorycode.json` dans le répertoire de votre projet :

   ```json title="sorycode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="sorycode.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à SoryCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `sorycode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration sorycode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration sorycode.
