Les plugins vous permettent d'étendre SoryCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de SoryCode.
...
- `.sorycode/plugins/` - Plugins au niveau du projet
- `~/.config/sorycode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/sorycode/node_modules/`.
...
1. Configuration globale (`~/.config/sorycode/sorycode.json`)
2. Configuration du projet (`sorycode.json`)
3. Répertoire global des plugins (`~/.config/sorycode/plugins/`)
4. Répertoire des plugins du projet (`.sorycode/plugins/`)
...
SoryCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK sorycode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre sorycode.
...
:::note
Si vous utilisez l'application de bureau SoryCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez sorycode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à sorycode :
...
L'assistant `tool` crée un outil personnalisé que sorycode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour sorycode aux côtés des outils intégrés.
