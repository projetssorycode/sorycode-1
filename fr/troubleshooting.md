Pour déboguer les problèmes avec SoryCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\sorycode\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `sorycode --log-level DEBUG`.

---

## Stockage

sorycode stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/sorycode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\sorycode`
...
SoryCode Desktop exécute un serveur SoryCode local (le side-car `sorycode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `SoryCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/sorycode/sorycode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
SoryCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/sorycode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\sorycode\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que SoryCode puisse le reconstruire.

1. Quittez complètement SoryCode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/sorycode`
- **Linux** : supprimez `~/.cache/sorycode` (ou exécutez `rm -rf ~/.cache/sorycode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\sorycode`

3. Redémarrez le bureau SoryCode.

---

### Résoudre les problèmes de connexion au serveur

SoryCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `sorycode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `SORYCODE_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `SORYCODE_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, SoryCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de SoryCode.

---

### Les notifications ne s'affichent pas

SoryCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour SoryCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau SoryCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application SoryCode Desktop) :

- `sorycode.settings.dat` (serveur de bureau par défaut URL)
- `sorycode.global.dat` et `sorycode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec SoryCode :
...
3. Assurez-vous d'avoir la dernière version avec `sorycode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `sorycode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\sorycode`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. sorycode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\sorycode`

2. Redémarrez sorycode pour réinstaller les derniers packages du fournisseur

Cela forcera sorycode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
sorycode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.