...
Le SDK sorycode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler sorycode par programme.
...
Créez une instance de sorycode :

```javascript
import { createOpencode } from "@sorycode-ai/sdk"
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `sorycode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createOpencode } from "@sorycode-ai/sdk"

const sorycode = await createOpencode({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${sorycode.server.url}`)

sorycode.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de sorycode, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createOpencodeClient } from "@sorycode-ai/sdk"
...
```typescript
import type { Session, Message, Part } from "@sorycode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*sorycode" },
})
