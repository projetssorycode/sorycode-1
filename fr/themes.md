Avec SoryCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, SoryCode utilise notre propre thème `sorycode`.
...
SoryCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que SoryCode corresponde à l'apparence de leur terminal
...
SoryCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/sorycode/themes/*.json` ou `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.sorycode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.sorycode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/sorycode/themes
vim ~/.config/sorycode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .sorycode/themes
vim .sorycode/themes/my-theme.json
```
