...
La commande `sorycode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client sorycode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `sorycode.local` |
...
Définissez `SORYCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `sorycode`, ou définissez `SORYCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `sorycode serve` et à `sorycode web`.
...
Lorsque vous exécutez `sorycode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur sorycode pour interagir avec sorycode par programmation.
:::

Cette architecture permet à sorycode de prendre en charge plusieurs clients et vous permet d'interagir avec sorycode par programme.

Vous pouvez exécuter `sorycode serve` pour démarrer un serveur autonome. Si vous avez le
sorycode TUI en cours d'exécution, `sorycode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins SoryCode [IDE](/docs/ide).
...
Le serveur sorycode expose les API suivantes.
