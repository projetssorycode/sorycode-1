SoryCode 通过 GitLab CI/CD 流水线或 GitLab Duo 与你的 GitLab 工作流集成。

在这两种情况下，SoryCode 都将在你的 GitLab Runner 上运行。

---

## GitLab CI

SoryCode 可以在常规的 GitLab 流水线中运行。你可以将其作为 [CI 组件](https://docs.gitlab.com/ee/ci/components/) 集成到流水线中。

这里我们使用的是社区创建的 SoryCode CI/CD 组件 — [nagyv/gitlab-sorycode](https://gitlab.com/nagyv/gitlab-opencode)。

---

### 功能特性

- **按任务自定义配置**：使用自定义配置目录来配置 SoryCode，例如 `./config/#custom-directory`，以便为每次 SoryCode 调用启用或禁用特定功能。
- **最小化配置**：CI 组件会在后台完成 SoryCode 的设置，你只需创建 SoryCode 配置和初始提示词即可。
...
1. 将你的 SoryCode 身份验证 JSON 作为文件类型的 CI 环境变量存储在 **Settings** > **CI/CD** > **Variables** 下。请确保将其标记为 "Masked and hidden"。
...
SoryCode 与你的 GitLab 工作流集成。
在评论中提及 `@sorycode`，SoryCode 将在你的 GitLab CI 流水线中执行任务。

---

### 功能特性

- **问题分类**：让 SoryCode 调查某个 issue 并为你解释。
- **修复与实现**：让 SoryCode 修复 issue 或实现某个功能。它会创建一个新分支，并提交包含更改的合并请求。
- **安全可靠**：SoryCode 在你的 GitLab Runner 上运行。

---

### 设置

SoryCode 在你的 GitLab CI/CD 流水线中运行，以下是设置所需的步骤：
...
以下是在 GitLab 中使用 SoryCode 的一些示例。

:::tip
你可以配置使用不同于 `@sorycode` 的触发词。
:::
...
- **修复 issue**
  ...
  SoryCode 会创建一个新分支，实现更改，并提交包含更改的合并请求。
