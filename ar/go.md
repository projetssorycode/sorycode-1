...
SoryCode Go هو اشتراك منخفض التكلفة بقيمة **10 دولارات شهرياً** يمنحك وصولاً موثوقاً إلى نماذج البرمجة المفتوحة الشائعة.

:::note
SoryCode Go حالياً في مرحلة تجريبية (beta).
:::

يعمل Go مثل أي مزود آخر في SoryCode. تشترك في SoryCode Go وتحصل على مفتاح API الخاص بك. إنه **اختياري تماماً** ولا تحتاج إلى استخدامه لاستخدام SoryCode.
...
:::tip
قمنا باختبار مجموعة مختارة من النماذج والمزودين الذين يعملون بشكل جيد مع SoryCode.
:::
...
يمنحك SoryCode Go الوصول إلى هذه النماذج مقابل **10 دولارات شهرياً**.

---

## كيف يعمل (How it works)

يعمل SoryCode Go مثل أي مزود آخر في SoryCode.

1. قم بتسجيل الدخول إلى **<a href={console}>SoryCode Zen</a>**، واشترك في Go، وانسخ مفتاح API الخاص بك.
2. قم بتشغيل الأمر `/connect` في واجهة TUI، وحدد `SoryCode Go`، والصق مفتاح API الخاص بك.
3. قم بتشغيل `/models` في واجهة TUI لرؤية قائمة النماذج المتاحة من خلال Go.

:::note
يمكن لعضو واحد فقط لكل مساحة عمل (workspace) الاشتراك في SoryCode Go.
:::
...
يتضمن SoryCode Go الحدود التالية:
...
SoryCode Go هي خطة اشتراك بقيمة **10 دولارات شهرياً**. أدناه الأسعار **لكل 1 مليون رمز (token)**.
...
يستخدم [model id](/docs/config/#models) في تكوين SoryCode الخاص بك التنسيق `sorycode-go/<model-id>`. على سبيل المثال، بالنسبة لـ Kimi K2.5، ستستخدم `sorycode-go/kimi-k2.5` في التكوين الخاص بك.
...
4. عدم وجود **قيود (lock-in)** من خلال السماح لك باستخدام أي مزود آخر مع SoryCode أيضاً.