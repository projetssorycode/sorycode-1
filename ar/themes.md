مع SoryCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم SoryCode سمتنا `sorycode`.
...
يأتي SoryCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق SoryCode مظهر terminal لديهم
...
يدعم SoryCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/sorycode/themes/*.json` أو `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.sorycode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.sorycode/themes/*.json`
