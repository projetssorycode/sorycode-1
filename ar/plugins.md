تتيح لك الإضافات توسيع SoryCode عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ SoryCode.
...
- `.sorycode/plugins/` - إضافات على مستوى المشروع
- `~/.config/sorycode/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/sorycode/node_modules/`.
...
1. إعدادات عامة (`~/.config/sorycode/sorycode.json`)
2. إعدادات المشروع (`sorycode.json`)
3. دليل الإضافات العام (`~/.config/sorycode/plugins/`)
4. دليل إضافات المشروع (`.sorycode/plugins/`)
...
يشغّل SoryCode الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ sorycode للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع sorycode.
...
:::note
إذا كنت تستخدم تطبيق SoryCode لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع sorycode من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى sorycode:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ sorycode استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ sorycode إلى جانب الأدوات المضمنة.
