لاستكشاف المشكلات في SoryCode وإصلاحها، ابدأ بالتحقق من السجلات والبيانات المحلية التي يخزنها على القرص.
...
- **Windows**: اضغط `WIN+R` والصق `%USERPROFILE%\.local\share\sorycode\log`

تتم تسمية ملفات السجل بطوابع زمنية (مثل `2025-01-09T123456.log`) ويتم الاحتفاظ بأحدث 10 ملفات سجل.

يمكنك ضبط مستوى السجل باستخدام خيار CLI `--log-level` للحصول على معلومات تصحيح أكثر تفصيلا. على سبيل المثال: `sorycode --log-level DEBUG`.

---

## التخزين

يخزن sorycode بيانات الجلسات وبيانات التطبيق الأخرى على القرص في:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: اضغط `WIN+R` والصق `%USERPROFILE%\.local\share\sorycode`
...
يشغل SoryCode Desktop خادما محليا لـ SoryCode (العملية الجانبية `sorycode-cli`) في الخلفية. معظم المشكلات سببها إضافة لا تعمل بشكل صحيح، أو ذاكرة تخزين مؤقت تالفة، أو إعداد خادم غير صحيح.

### فحوصات سريعة

- أغلق التطبيق تماما ثم أعد تشغيله.
- إذا عرض التطبيق شاشة خطأ، انقر **Restart** وانسخ تفاصيل الخطأ.
- على macOS فقط: قائمة `SoryCode` -> **Reload Webview** (يفيد إذا كانت الواجهة فارغة/متجمدة).
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (أو `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (عمليات تثبيت أقدم): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: اضغط `WIN+R` والصق `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
يمكن لـ SoryCode أيضا تحميل إضافات محلية من القرص. انقلها مؤقتا إلى مكان آخر (أو أعد تسمية المجلد) ثم أعد تشغيل تطبيق سطح المكتب:

- **إضافات عامة**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: اضغط `WIN+R` والصق `%USERPROFILE%\.config\sorycode\plugins`
...
إذا لم يساعد تعطيل الإضافات (أو كانت عملية تثبيت إضافة عالقة)، فامسح ذاكرة التخزين المؤقت حتى يتمكن SoryCode من إعادة بنائها.

1. أغلق SoryCode Desktop تماما.
2. احذف دليل ذاكرة التخزين المؤقت:

- **macOS**: Finder -> `Cmd+Shift+G` -> الصق `~/.cache/sorycode`
- **Linux**: احذف `~/.cache/sorycode` (أو شغّل `rm -rf ~/.cache/sorycode`)
- **Windows**: اضغط `WIN+R` والصق `%USERPROFILE%\.cache\sorycode`

3. أعد تشغيل SoryCode Desktop.

---

### إصلاح مشكلات اتصال الخادم

يمكن لـ SoryCode Desktop إما تشغيل خادمه المحلي (افتراضيا) أو الاتصال بعنوان URL لخادم قمت بتهيئته.
...
إذا كان `sorycode.json(c)` يحتوي على قسم `server`، فأزله مؤقتا ثم أعد تشغيل تطبيق سطح المكتب.

#### تحقق من متغيرات البيئة

إذا كان `SORYCODE_PORT` مضبوطا في بيئتك، فسيحاول تطبيق سطح المكتب استخدام ذلك المنفذ للخادم المحلي.

- أزل ضبط `SORYCODE_PORT` (أو اختر منفذا متاحا) ثم أعد التشغيل.
...
على Windows، يتطلب SoryCode Desktop وجود **WebView2 Runtime** الخاصة بـ Microsoft Edge. إذا فتح التطبيق نافذة فارغة أو لم يبدأ، فقم بتثبيت/تحديث WebView2 ثم جرّب مجددا.

---

### Windows: مشكلات الأداء العامة

إذا كنت تواجه بطءا في الأداء، أو مشكلات في الوصول إلى الملفات، أو مشكلات في terminal على Windows، فجرّب استخدام [WSL (نظام Windows الفرعي لـ Linux)](/docs/windows-wsl). يوفر WSL بيئة Linux تعمل بسلاسة أكبر مع ميزات SoryCode.

---

### الإشعارات لا تظهر

لا يعرض SoryCode Desktop إشعارات النظام إلا عندما:

- تكون الإشعارات مفعلة لـ SoryCode في إعدادات نظام التشغيل، و
...
1. أغلق SoryCode Desktop.
2. اعثر على هذه الملفات واحذفها (توجد في دليل بيانات تطبيق SoryCode Desktop):

- `sorycode.settings.dat` (عنوان URL الافتراضي لخادم سطح المكتب)
- `sorycode.global.dat` و `sorycode.workspace.*.dat` (حالة الواجهة مثل الخوادم/المشاريع الأخيرة)
...
إذا كنت تواجه مشكلات مع SoryCode:
...
3. تأكد من أنك تستخدم أحدث إصدار عبر `sorycode upgrade`
...
لمعرفة النماذج التي لديك صلاحية الوصول إليها، شغّل `sorycode models`
...
2. إذا استمرت المشكلة، فجرّب مسح الإعدادات المخزنة لديك:
   ...
   على Windows، اضغط `WIN+R` واحذف: `%USERPROFILE%\.local\share\sorycode`
...
إذا واجهت أخطاء في استدعاءات API، فقد يكون السبب حزم مزوّد قديمة. يقوم sorycode بتثبيت حزم المزوّد (OpenAI و Anthropic و Google وغير ذلك) ديناميكيا عند الحاجة ويقوم بتخزينها مؤقتا محليا.
...
1. امسح ذاكرة التخزين المؤقت لحزم المزوّد:
   ...
   على Windows، اضغط `WIN+R` واحذف: `%USERPROFILE%\.cache\sorycode`

2. أعد تشغيل sorycode لإعادة تثبيت أحدث حزم المزوّد

سيجبر ذلك sorycode على تنزيل أحدث إصدارات حزم المزوّد، وهو ما يحل غالبا مشكلات التوافق مع معاملات النماذج وتغييرات API.
...
سيكتشف sorycode ما إذا كنت تستخدم Wayland ويفضل `wl-clipboard`، وإلا فسيحاول العثور على أدوات الحافظة بالترتيب التالي: `xclip` ثم `xsel`.