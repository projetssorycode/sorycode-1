يدعم SoryCode بروتوكول [Agent Client Protocol](https://agentclientprotocol.com) أو (ACP)، مما يتيح لك استخدامه مباشرة في المحررات وبيئات التطوير المتكاملة (IDEs) المتوافقة.
...
لاستخدام SoryCode عبر ACP، اضبط محررك ليشغّل الأمر `sorycode acp`.

يشغّل هذا الأمر SoryCode كعملية فرعية متوافقة مع ACP تتواصل مع محررك عبر JSON-RPC باستخدام stdio.
...
لفتحه، اختر الوكيل الجديد 'SoryCode' من محدد الوكلاء في AI Chat.
...
لاستخدام SoryCode كوكيل ACP في [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)، أضف التالي إلى إعدادات Neovim:
...
يضبط هذا الإعداد CodeCompanion لاستخدام SoryCode كوكيل ACP للدردشة.

إذا احتجت إلى تمرير متغيرات البيئة (مثل `SORYCODE_API_KEY`)، فارجع إلى [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) ضمن توثيق CodeCompanion.nvim للاطلاع على التفاصيل كاملة.

## الدعم

يعمل SoryCode عبر ACP بالطريقة نفسها التي يعمل بها في terminal. جميع الميزات مدعومة:
...
- خوادم MCP المضبوطة في إعدادات SoryCode
