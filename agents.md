...
There are two types of agents in SoryCode; primary agents and subagents.
...
SoryCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
SoryCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

SoryCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `sorycode.json` config file:
...
If no temperature is specified, SoryCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global SoryCode config and the project specific config.
...
The model ID in your SoryCode config uses the format `provider/model-id`. For example, if you're using [SoryCode Zen](/docs/zen), you would use `sorycode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `sorycode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/projetssorycode/sorycode-1).
:::
