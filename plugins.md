Plugins allow you to extend SoryCode by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify SoryCode's default behavior.
...
- `.sorycode/plugins/` - Project-level plugins
- `~/.config/sorycode/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/sorycode/node_modules/`.
...
3. Global plugin directory (`~/.config/sorycode/plugins/`)
4. Project plugin directory (`.sorycode/plugins/`)
...
SoryCode runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An sorycode SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend sorycode.
...
:::note
If you’re using the SoryCode desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent sorycode from reading `.env` files:
...
Plugins can also add custom tools to sorycode:
...
The `tool` helper creates a custom tool that sorycode can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to sorycode alongside built-in tools.
