Mit Plugins können Sie SoryCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von SoryCode zu ändern.
...
- `.sorycode/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/sorycode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/sorycode/sorycode.json`)
2. Projektkonfiguration (`sorycode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/sorycode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.sorycode/plugins/`)
...
SoryCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein SoryCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie SoryCode erweitern können.
...
:::note
Wenn Sie die SoryCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass SoryCode `.env`-Dateien liest:
...
Plugins können SoryCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das SoryCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für SoryCode zur Verfügung.
