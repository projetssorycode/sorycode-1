Mit SoryCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet SoryCode das `sorycode`-Theme.
...
SoryCode liefert mehrere eingebaute Themes mit.
...
- SoryCode optisch ans Terminal anpassen wollen
...
SoryCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/sorycode/themes/*.json` oder `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.sorycode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.sorycode/themes/*.json`
