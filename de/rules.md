Du kannst sorycode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in sorycode den Befehl `/init` aus.
...
So kann sorycode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht sorycode sie zu erweitern.
...
sorycode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/sorycode/AGENTS.md` speichern.
Diese gelten in allen sorycode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt SoryCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/sorycode/AGENTS.md` existiert)
...
Beim Start sucht sorycode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/sorycode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `sorycode.json` oder in `~/.config/sorycode/sorycode.json` angeben.
...
sorycode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit sorycode.json

Empfohlen ist das `instructions`-Feld in der `sorycode.json`:
...
Du kannst sorycode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `sorycode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::