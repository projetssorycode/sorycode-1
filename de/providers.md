...
SoryCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer SoryCode-Konfiguration.
...
in `~/.local/share/sorycode/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem SoryCode anpassen
config.
...
SoryCode Zen ist eine Liste von Modellen, die vom SoryCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit SoryCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit SoryCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie SoryCode aus und gehen Sie zu [sorycode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in SoryCode und ist völlig optional.

---

## SoryCode Go

SoryCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom SoryCode-Team getestet und verifiziert wurden, dass sie gut mit SoryCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `SoryCode Go` und gehen Sie zu [sorycode.ai/auth](https://opencode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit SoryCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie SoryCode ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `sorycode.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in SoryCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit SoryCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre SoryCode-Konfiguration hinzufügen.
...
um das Token im SoryCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
SoryCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von SoryCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`sorycode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `sorycode.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit SoryCode:
...
So verwenden Sie Google Vertex AI mit SoryCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie SoryCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über SoryCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede SoryCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede SoryCode-Konversation als separate Sitzung aufgeführt.
...
Sie können SoryCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können SoryCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können SoryCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für SoryCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit SoryCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in SoryCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in SoryCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
SoryCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom SoryCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>SoryCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren SoryCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre SoryCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre SoryCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit SoryCode:
...
Sie können Modelle auch über Ihre SoryCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit SoryCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `sorycode.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann SoryCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `sorycode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die SoryCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer SoryCode-Konfiguration übereinstimmt.
