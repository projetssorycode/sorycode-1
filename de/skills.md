Agent Skills erlauben SoryCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
SoryCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft SoryCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.sorycode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/sorycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.sorycode/skills/git-release/SKILL.md` zum Beispiel so:
...
SoryCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `sorycode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `sorycode.json`):
