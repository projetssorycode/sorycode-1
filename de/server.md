...
Der Befehl `sorycode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein sorycode-Client nutzen kann.
...
Setze `SORYCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `sorycode`, kann aber mit `SORYCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `sorycode serve` und `sorycode web`.
...
Wenn du `sorycode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den sorycode-Server, um sorycode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `sorycode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `sorycode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die SoryCode-[IDE](/docs/ide)-Plugins.
...
Der sorycode-Server stellt folgende APIs bereit.
