Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von SoryCode wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.sorycode/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/sorycode/tools/` platzieren.
