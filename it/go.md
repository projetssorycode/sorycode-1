...
SoryCode Go è un abbonamento a basso costo di **$10/mese** che ti offre un accesso affidabile ai modelli di coding open source più popolari.

:::note
SoryCode Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in SoryCode. Ti abboni a SoryCode Go e ottieni la tua chiave API. È **completamente opzionale** e non è necessario utilizzarlo per usare SoryCode.
...
SoryCode Go ti dà accesso a questi modelli per **$10/mese**.

---

## Come funziona

SoryCode Go funziona come qualsiasi altro provider in SoryCode.

1. Accedi a **<a href={console}>SoryCode Zen</a>**, abbonati a Go e copia la tua chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona `SoryCode Go` e incolla la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a SoryCode Go.
:::
...
SoryCode Go include i seguenti limiti:
...
SoryCode Go è un piano di abbonamento da **$10/mese**. Di seguito sono riportati i prezzi **per 1M di token**.
...
Il [model id](/docs/config/#models) nella tua configurazione SoryCode utilizza il formato `sorycode-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti `sorycode-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato SoryCode Go per:
...
4. Non avere **alcun lock-in** permettendoti di utilizzare qualsiasi altro provider con SoryCode.