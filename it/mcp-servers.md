Puoi aggiungere strumenti esterni a SoryCode usando il _Model Context Protocol_, o MCP. SoryCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [SoryCode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/sorycode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
SoryCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, SoryCode:
...
Se il server richiede autenticazione, SoryCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `sorycode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, SoryCode memorizzera' i token in modo sicuro in `~/.local/share/sorycode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in SoryCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione SoryCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare SoryCode al tuo account Sentry.
