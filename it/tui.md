...
SoryCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire SoryCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di SoryCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a SoryCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da SoryCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `sorycode.json`, che configura il comportamento di server/runtime.
...
Usa `SORYCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
