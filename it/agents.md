...
In SoryCode esistono due tipi di agenti: agenti primari e subagenti.
...
SoryCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
SoryCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

SoryCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `sorycode.json`:
...
- Globale: `~/.config/sorycode/agents/`
- Per progetto: `.sorycode/agents/`
...
Se non specifichi una temperature, SoryCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di SoryCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione SoryCode usa il formato `provider/model-id`. Per esempio, se stai usando [SoryCode Zen](/docs/zen), useresti `sorycode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `sorycode models` per vedere la lista dei modelli disponibili.
:::
