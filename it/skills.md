Le skill degli agenti permettono a SoryCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
SoryCode cerca in queste posizioni:

- Config di progetto: `.sorycode/skills/<name>/SKILL.md`
- Config globale: `~/.config/sorycode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, SoryCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.sorycode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/sorycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.sorycode/skills/git-release/SKILL.md` cosi':
...
SoryCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `sorycode.json`:
...
**Per agenti integrati** (in `sorycode.json`):
