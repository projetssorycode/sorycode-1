Per diagnosticare problemi con SoryCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\sorycode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `sorycode --log-level DEBUG`.

---

## Archiviazione

sorycode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\sorycode`
...
SoryCode Desktop esegue in background un server locale di SoryCode (il sidecar `sorycode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `SoryCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
SoryCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\sorycode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che SoryCode possa ricostruirla.

1. Chiudi completamente SoryCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/sorycode`
- **Linux**: elimina `~/.cache/sorycode` (oppure esegui `rm -rf ~/.cache/sorycode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\sorycode`

3. Riavvia SoryCode Desktop.

---

### Risolvi problemi di connessione al server

SoryCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `sorycode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `SORYCODE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `SORYCODE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, SoryCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di SoryCode.

---

### Notifiche non visualizzate

SoryCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per SoryCode nelle impostazioni del sistema operativo, e
...
1. Chiudi SoryCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app SoryCode Desktop):

- `sorycode.settings.dat` (desktop default server URL)
- `sorycode.global.dat` e `sorycode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con SoryCode:
...
Per capire a quali modelli hai accesso, esegui `sorycode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\sorycode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. sorycode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\sorycode`

2. Riavvia sorycode per reinstallare i pacchetti provider piu recenti

Questo forzera sorycode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
sorycode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.