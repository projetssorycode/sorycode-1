...
Il comando `sorycode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client sorycode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `sorycode.local` |
...
Imposta `SORYCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `sorycode`, oppure imposta `SORYCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `sorycode serve` sia per `sorycode web`.
...
Quando esegui `sorycode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server sorycode per interagire con sorycode in modo programmatico.
:::

Questa architettura permette a sorycode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `sorycode serve` per avviare un server standalone. Se la TUI di sorycode e gia in esecuzione, `sorycode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di SoryCode.
...
Il server sorycode espone le seguenti API.
