I plugin ti permettono di estendere SoryCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di SoryCode.
...
- `.sorycode/plugins/` - plugin a livello progetto
- `~/.config/sorycode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/sorycode/node_modules/`.
...
1. Config globale (`~/.config/sorycode/sorycode.json`)
2. Config di progetto (`sorycode.json`)
3. Directory plugin globale (`~/.config/sorycode/plugins/`)
4. Directory plugin di progetto (`.sorycode/plugins/`)
...
SoryCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di sorycode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere sorycode.
...
:::note
Se usi l'app desktop di SoryCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a sorycode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a sorycode:
...
L'helper `tool` crea uno strumento personalizzato che sorycode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in sorycode insieme agli strumenti integrati.
