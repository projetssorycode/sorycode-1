:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di sorycode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in sorycode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di sorycode.
...
sorycode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.sorycode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `sorycode.json`:
...
- Globale: `~/.config/sorycode/modes/`
- Progetto: `.sorycode/modes/`
...
Se non viene specificata alcuna temperatura, sorycode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di sorycode sia per quella specifica del progetto.
...
Crea file di modalita in `.sorycode/modes/` per modalita specifiche del progetto oppure in `~/.config/sorycode/modes/` per modalita globali:
