...
SoryCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj SoryCode konfiguraciji.
...
u `~/.local/share/sorycode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem SoryCode
config.
...
SoryCode Zen je lista modela koje je obezbedio SoryCode tim koji su bili
testirano i potvrđeno da dobro radi sa SoryCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa SoryCode Zen.
:::
...
Radi kao i svaki drugi provajder u SoryCode i potpuno je opcionalan za korištenje.

---

## SoryCode Go

SoryCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža SoryCode tim i koji su testirani i verificirani da dobro rade s SoryCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `SoryCode Go` i idite na [sorycode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s SoryCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete sorycode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `sorycode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u SoryCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi sorycode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju sorycode konfiguraciju.
...
da pohrani token u sorycode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
SoryCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate SoryCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`sorycode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `sorycode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s sorycode:
...
Za korištenje Google Vertex AI s SoryCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće sorycode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem sorycodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki SoryCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki SoryCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati sorycode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati sorycode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati sorycode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za SoryCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s SoryCode:
...
3. Kopirajte API ključ za korištenje u SoryCode.
...
6. **Važno**: Prije upotrebe modela oblaka u SoryCode, morate lokalno povući informacije o modelu:
...
SoryCode Zen je lista testiranih i verifikovanih modela koju je obezbedio SoryCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>SoryCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **SoryCode Zen**.

```txt
   /connect
```

3. Unesite svoj SoryCode API ključ.
...
Također možete dodati dodatne modele putem vaše sorycode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše sorycode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju sorycode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s sorycode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `sorycode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju SoryCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `sorycode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite SoryCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj sorycode konfiguraciji.
