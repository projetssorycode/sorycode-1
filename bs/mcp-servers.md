Možete dodati vanjske alate u SoryCode koristeći _Model Context Protocol_, ili MCP. SoryCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [SoryCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/sorycode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
SoryCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, SoryCode će:
...
Ako server zahtijeva autentifikaciju, SoryCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `sorycode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, SoryCode sigurno čuva tokene u `~/.local/share/sorycode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u SoryCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz SoryCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete SoryCode sa vašim Sentry nalogom.
