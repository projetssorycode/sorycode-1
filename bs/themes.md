U SoryCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, SoryCode koristi nasu `sorycode` temu.
...
SoryCode dolazi sa vise ugradenih tema.
...
- Zele da SoryCode odgovara izgledu njihovog terminala
...
SoryCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/sorycode/themes/*.json` ili `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.sorycode/themes/*.json`
4. **Trenutni radni direktorij** - `./.sorycode/themes/*.json`
