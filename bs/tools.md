Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. SoryCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u SoryCode.
...
:::note
Ovaj alat je dostupan samo kada je `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `SORYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz SoryCode provajdera ili kada je varijabla `SORYCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju SoryCode:
