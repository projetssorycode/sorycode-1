Možete konfigurirati SoryCode koristeći JSON konfiguracijski fajl.

---

## Format

SoryCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/sorycode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/sorycode/sorycode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SORYCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`sorycode.json` u projektu) - postavke specifične za projekat
5. **`.sorycode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SORYCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.sorycode` i `~/.config/sorycode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/sorycode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu SoryCode konfiguraciju u `~/.config/sorycode/sorycode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/sorycode/tui.json`.
...
Dodajte `sorycode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se SoryCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SORYCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SORYCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.sorycode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.sorycode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`sorycode.ai/config.json`**](https://opencode.ai/config.json).

TUI konfiguracija koristi [**`sorycode.ai/tui.json`**](https://opencode.ai/tui.json).
...
Koristite `SORYCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `sorycode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `sorycode serve` i `sorycode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš SoryCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `sorycode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj SoryCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, SoryCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/sorycode/agents/` ili `.sorycode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, SoryCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`sorycode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/sorycode/commands/` ili `.sorycode/commands/`. [Saznajte više ovdje](/docs/commands).
...
SoryCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, SoryCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju SoryCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.sorycode/plugins/` ili `~/.config/sorycode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite SoryCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
