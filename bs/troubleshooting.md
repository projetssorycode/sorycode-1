Da biste otklonili probleme s SoryCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sorycode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `sorycode --log-level DEBUG`.

---

## Pohrana

SoryCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\sorycode`
...
SoryCode Desktop pokreće lokalni SoryCode server (`sorycode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `SoryCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (ili `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sorycode\sorycode.jsonc`
...
SoryCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\sorycode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga SoryCode mogao ponovo izgraditi.

1. Potpuno zatvorite SoryCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/sorycode`
- **Linux**: obrišite `~/.cache/sorycode` (ili pokrenite `rm -rf ~/.cache/sorycode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\sorycode`

3. Ponovo pokrenite SoryCode Desktop.

---

### Rješavanje problema sa vezom na serveru

SoryCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `sorycode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SORYCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SORYCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, SoryCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa SoryCode karakteristikama.

---

### Obavještenja se ne prikazuju

SoryCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za SoryCode u postavkama vašeg OS-a, i
...
1. Zatvorite SoryCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka SoryCode Desktop aplikacije):

- `sorycode.settings.dat` (URL zadanog servera za desktop)
- `sorycode.global.dat` i `sorycode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `sorycode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\sorycode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. SoryCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\sorycode`

2. Ponovo pokrenite SoryCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti SoryCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
SoryCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.