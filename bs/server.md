...
Komanda `sorycode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji sorycode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `sorycode.local` |
...
Postavite `SORYCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `sorycode`, ili postavite `SORYCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `sorycode serve` i za `sorycode web`.
...
Kada pokrenete `sorycode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite sorycode server za programsku interakciju sa sorycode-om.
:::

Ova arhitektura omogucava sorycode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `sorycode serve` da startate standalone server. Ako je sorycode TUI vec pokrenut, `sorycode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste SoryCode [IDE](/docs/ide) pluginovi.
...
sorycode server izlaže sljedece API-je.
