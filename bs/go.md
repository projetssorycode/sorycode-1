...
SoryCode Go je povoljna pretplata od **$10/mjesečno** koja vam daje pouzdan pristup popularnim otvorenim modelima kodiranja.

:::note
SoryCode Go je trenutno u beta fazi.
:::

Go funkcioniše kao bilo koji drugi pružatelj usluga u SoryCode-u. Pretplatite se na SoryCode Go i
...
:::tip
Testirali smo odabranu grupu modela i pružatelja usluga koji dobro rade sa SoryCode-om.
:::
...
SoryCode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako to funkcioniše

SoryCode Go funkcioniše kao bilo koji drugi pružatelj usluga u SoryCode-u.

1. Prijavite se na **<a href={console}>SoryCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite `/connect` komandu u TUI-ju, odaberite `SoryCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih putem Go.

:::note
Samo jedan član po radnom prostoru se može pretplatiti na SoryCode Go.
:::
...
SoryCode Go uključuje sljedeća ograničenja:
...
SoryCode Go je pretplatnički plan od **$10/mjesečno**. Ispod su cijene **po 1M tokena**.
...
[Model id](/docs/config/#models) u vašoj SoryCode konfiguraciji
koristi format `sorycode-go/<model-id>`. Na primjer, za Kimi K2.5, biste
koristili `sorycode-go/kimi-k2.5` u vašoj konfiguraciji.
...
4. Nemamo **zaključavanja (lock-in)** omogućavajući vam da koristite bilo kojeg drugog pružatelja usluga sa SoryCode-om također.