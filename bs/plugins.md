Dodaci vam omogućavaju da proširite SoryCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja SoryCode.
...
- `.sorycode/plugins/` - Dodaci na nivou projekta
- `~/.config/sorycode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/sorycode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/sorycode/sorycode.json`)
2. Konfiguracija projekta (`sorycode.json`)
3. Globalni direktorij dodataka (`~/.config/sorycode/plugins/`)
4. Direktorij dodataka projekta (`.sorycode/plugins/`)
...
SoryCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje SoryCode.
...
Spriječite sorycode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u sorycode:
...
Pomoćnik `tool` kreira prilagođeni alat koji sorycode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za sorycode zajedno sa ugrađenim alatima.
