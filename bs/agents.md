...
Postoje dvije vrste agenata u SoryCode: primarni agenti i podagenti.
...
SoryCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
SoryCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

SoryCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `sorycode.json`:
...
- Globalno: `~/.config/sorycode/agents/`
- Po projektu: `.sorycode/agents/`
...
Ako temperatura nije navedena, SoryCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu SoryCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj SoryCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [SoryCode Zen](/docs/zen), koristili biste `sorycode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `sorycode models` da vidite listu dostupnih modela.
:::
