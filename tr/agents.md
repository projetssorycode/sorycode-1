...
sorycode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
sorycode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
sorycode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

sorycode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `sorycode.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse sorycode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global sorycode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
sorycode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [SoryCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `sorycode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `sorycode models` komutunu çalıştırın.
:::
