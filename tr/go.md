...
SoryCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
SoryCode Go şu anda beta aşamasındadır.
:::

Go, SoryCode içindeki diğer sağlayıcılar gibi çalışır. SoryCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve SoryCode'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
SoryCode ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
SoryCode Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

SoryCode Go, SoryCode'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>SoryCode Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `SoryCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanılabilen modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Çalışma alanı başına yalnızca bir üye SoryCode Go'ya abone olabilir.
:::
...
SoryCode Go aşağıdaki sınırları içerir:
...
SoryCode Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
SoryCode yapılandırmanızdaki [model kimliği](/docs/config/#models), `sorycode-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `sorycode-go/kimi-k2.5` kullanırsınız.
...
SoryCode Go'yu şu amaçlarla oluşturduk:
...
4. SoryCode ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.