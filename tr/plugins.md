Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek SoryCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya SoryCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.sorycode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/sorycode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/sorycode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/sorycode/sorycode.json`)
2. Project config (`sorycode.json`)
3. Global eklenti dizini (`~/.config/sorycode/plugins/`)
4. Proje eklenti dizini (`.sorycode/plugins/`)
...
SoryCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir SoryCode SDK istemcisi.
...
SoryCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
SoryCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
SoryCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca SoryCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, SoryCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra SoryCode'a da hazır olacaktır.
