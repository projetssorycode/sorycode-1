...
SoryCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

SoryCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
SoryCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
SoryCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
SoryCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `sorycode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `SORYCODE_TUI_CONFIG` kullanın.
