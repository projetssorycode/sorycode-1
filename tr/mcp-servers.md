_Model Bağlam Protokolü_ veya MCP kullanarak sorycode'a harici araçlar ekleyebilirsiniz. sorycode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [sorycode Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/sorycode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
sorycode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde sorycode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa sorycode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `sorycode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından sorycode, belirteçleri `~/.local/share/sorycode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra sorycode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi sorycode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve sorycode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
