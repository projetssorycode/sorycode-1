...
SoryCode Zen, sorycode ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
SoryCode Zen şu anda beta aşamasındadır.
:::

Zen, sorycode'daki diğer sağlayıcılar gibi çalışır. SoryCode Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve sorycode kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
sorycode ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
SoryCode Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

SoryCode Zen, sorycode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>SoryCode Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, SoryCode Zen'i seçin ve API anahtarınızı yapıştırın.
...
sorycode yapılandırmanızdaki [model kimliği](/docs/config/#models) `sorycode/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `sorycode/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için SoryCode'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için SoryCode'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
SoryCode Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman SoryCode ile başka bir sağlayıcıyı kullanmanıza izin vermek.