sorycode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak sorycode kendi `sorycode` temasını kullanır.
...
sorycode birden fazla yerleşik temayla gelir.
...
- sorycode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
sorycode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/sorycode/themes/*.json` veya `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.sorycode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.sorycode/themes/*.json`
