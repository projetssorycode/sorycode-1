Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. sorycode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
sorycode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `SORYCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca sorycode provider kullanılırken veya `SORYCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

sorycode başlatırken etkinleştirmek için:
