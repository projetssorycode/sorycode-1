...
`sorycode serve` komutu, sorycode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `sorycode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `SORYCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `sorycode` değeridir; değiştirmek isterseniz `SORYCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `sorycode serve` hem de `sorycode web` için geçerlidir.
...
`sorycode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
sorycode ile programatik etkileşim için sorycode sunucusunu kullanın.
:::

Bu mimari, sorycode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`sorycode serve` ile bağımsız bir sunucu başlatabilirsiniz. sorycode TUI açıksa, `sorycode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum sorycode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
sorycode sunucusu aşağıdaki API'leri sunar.
