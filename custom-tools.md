Custom tools are functions you create that the LLM can call during conversations. They work alongside sorycode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.sorycode/tools/` directory of your project.
- Or globally, by placing them in `~/.config/sorycode/tools/`.
