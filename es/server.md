...
El comando `sorycode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente sorycode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `sorycode.local` |
...
Configure `SORYCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `sorycode`, o configure `SORYCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `sorycode serve` como a `sorycode web`.
...
Cuando ejecuta `sorycode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor sorycode para interactuar con sorycode mediante programación.
:::

Esta arquitectura permite que sorycode admita múltiples clientes y le permite interactuar con sorycode mediante programación.

Puede ejecutar `sorycode serve` para iniciar un servidor independiente. Si tienes el
sorycode TUI ejecutándose, `sorycode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos SoryCode [IDE](/docs/ide).
...
El servidor sorycode expone las siguientes API.
