Con SoryCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, SoryCode usa nuestro propio tema `sorycode`.
...
SoryCode viene con varios temas integrados.
...
- Quiere que SoryCode coincida con la apariencia de su terminal
...
SoryCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/sorycode/themes/*.json` o `$XDG_CONFIG_HOME/sorycode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.sorycode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.sorycode/themes/*.json`
