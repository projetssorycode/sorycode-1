Las herramientas permiten que LLM realice acciones en su código base. SoryCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en SoryCode.
...
:::note
Esta herramienta solo está disponible cuando `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (o `SORYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor SoryCode o cuando la variable de entorno `SORYCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar SoryCode:

```bash
SORYCODE_ENABLE_EXA=1 sorycode
```
