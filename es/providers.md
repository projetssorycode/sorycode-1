...
SoryCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración SoryCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu SoryCode
configuración.
...
SoryCode Zen es una lista de modelos proporcionados por el equipo SoryCode que han sido
probado y verificado para funcionar bien con SoryCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con SoryCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione sorycode y diríjase a [sorycode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en SoryCode y su uso es completamente opcional.

---

## SoryCode Go

SoryCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de SoryCode que han sido
probado y verificado para funcionar bien con SoryCode.

1. Ejecute el comando `/connect` en TUI, seleccione `SoryCode Go` y diríjase a [sorycode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con SoryCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta sorycode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `sorycode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en SoryCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que sorycode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración sorycode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `SoryCode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación sorycode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
SoryCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear SoryCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `sorycode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [sorycode-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuración

Personalizar a través de `sorycode.json`:
...
Para utilizar su suscripción GitHub Copilot con sorycode:
...
Para utilizar Google Vertex AI con SoryCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta sorycode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de sorycode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [sorycode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación SoryCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación SoryCode enumerada como una sesión separada.
...
Puede configurar sorycode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar sorycode para usar modelos locales a través de LM Studio.
...
Puedes configurar sorycode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para SoryCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con SoryCode:
...
3. Copie la clave API para usarla en SoryCode.
...
6. **Importante**: Antes de usar modelos en la nube en SoryCode, debe obtener la información del modelo localmente:
...
SoryCode Zen es una lista de modelos probados y verificados proporcionada por el equipo SoryCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>SoryCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **SoryCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave SoryCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración sorycode.
...
5. También puedes personalizarlos a través de tu configuración sorycode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con SoryCode:
...
También puede personalizar modelos a través de su configuración sorycode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con sorycode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `sorycode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a SoryCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `sorycode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración sorycode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración sorycode.
