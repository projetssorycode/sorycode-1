Los complementos le permiten extender SoryCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de SoryCode.
...
- `.sorycode/plugins/` - Complementos a nivel de proyecto
- `~/.config/sorycode/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/sorycode/node_modules/`.
...
1. Configuración global (`~/.config/sorycode/sorycode.json`)
2. Configuración del proyecto (`sorycode.json`)
3. Directorio global de complementos (`~/.config/sorycode/plugins/`)
4. Directorio de complementos del proyecto (`.sorycode/plugins/`)
...
SoryCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK sorycode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar sorycode.
...
:::note
Si está utilizando la aplicación de escritorio SoryCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que sorycode lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a sorycode:
...
El ayudante `tool` crea una herramienta personalizada a la que sorycode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para sorycode junto con las herramientas integradas.
