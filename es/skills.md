Las habilidades del agente permiten a SoryCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
SoryCode busca estas ubicaciones:

- Configuración del proyecto: `.sorycode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/sorycode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, SoryCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.sorycode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/sorycode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.sorycode/skills/git-release/SKILL.md` así:
...
SoryCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `sorycode.json`:
...
**Para agentes integrados** (en `sorycode.json`):
