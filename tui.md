...
SoryCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running SoryCode starts the TUI for the current directory.
...
When using the SoryCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to SoryCode. Allows you to select from available providers and add their API keys.
...
Exit SoryCode. _Aliases_: `/quit`, `/q`
...
This is separate from `sorycode.json`, which configures server/runtime behavior.
...
Use `SORYCODE_TUI_CONFIG` to load a custom TUI config path.
