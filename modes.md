:::caution
Modes are now configured through the `agent` option in the sorycode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in sorycode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the sorycode config.
...
sorycode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.sorycode/plans/*.md` to detail the plan itself
...
Configure modes in your `sorycode.json` config file:
...
both the global sorycode config and the project specific config.
...
Create mode files in `.sorycode/modes/` for project-specific modes or `~/.config/sorycode/modes/` for global modes:
