...
sorycode предоставляет интерактивный terminal интерфейс или TUI для работы над вашими проектами с помощью LLM.

Запуск sorycode запускает TUI для текущего каталога.
...
При использовании sorycode TUI вы можете ввести `/`, а затем имя команды, чтобы быстро выполнить действия. Например:
...
Добавьте провайдера в sorycode. Позволяет выбирать из доступных поставщиков и добавлять их ключи API.
...
Выйдите из sorycode. _Псевдонимы_: `/quit`, `/q`
...
Это отдельный файл от `sorycode.json`, который настраивает поведение сервера/выполнения.
...
Используйте `SORYCODE_TUI_CONFIG` для загрузки пользовательского пути конфигурации TUI.
