...
SoryCode Zen — это список протестированных и проверенных моделей, предоставленный командой SoryCode.

:::note
SoryCode Zen в настоящее время находится в стадии бета-тестирования.
:::

Zen работает как любой другой провайдер в SoryCode. Вы входите в SoryCode Zen и получаете
...
:::tip
Мы протестировали избранную группу моделей и поставщиков, которые хорошо работают с sorycode.
:::
...
SoryCode Zen — это шлюз искусственного интеллекта, который дает вам доступ к этим моделям.

---

## Как это работает

SoryCode Zen работает так же, как и любой другой поставщик SoryCode.

1. Вы входите в систему **<a href={console}>SoryCode Zen</a>**, добавляете платежные
   данные и копируете свой ключ API.
2. Вы запускаете команду `/connect` в TUI, выбираете SoryCode Zen и вставляете свой ключ API.
...
[модель id](/docs/config/#models) в вашей конфигурации sorycode
использует формат `sorycode/<model-id>`. Например, для Кодекса GPT 5.2 вы должны
используйте `sorycode/gpt-5.2-codex` в вашей конфигурации.
...
- MiniMax M2.5 Free доступен на SoryCode в течение ограниченного времени. Команда использует это время для сбора отзывов и улучшения модели.
- Big Pickle — это стелс-модель, которая доступна бесплатно на SoryCode в течение ограниченного времени. Команда использует это время для сбора отзывов и улучшения модели.
...
4. Исключить **привязку**, позволяя использовать его с любым другим кодинг-агентом. И всегда позволяя вам использовать любого другого провайдера с SoryCode.