...
SoryCode Go — это недорогая подписка за **$10/месяц**, которая предоставляет надежный доступ к популярным открытым моделям для кодинга.

:::note
SoryCode Go в настоящее время находится в бета-версии.
:::

Go работает как любой другой провайдер в SoryCode. Вы подписываетесь на SoryCode Go и получаете свой API ключ. Это **полностью опционально**, и вам не нужно использовать его, чтобы пользоваться SoryCode.
...
:::tip
Мы протестировали избранную группу моделей и провайдеров, которые хорошо работают с SoryCode.
:::
...
SoryCode Go дает вам доступ к этим моделям за **$10/месяц**.

---

## Как это работает

SoryCode Go работает как любой другой провайдер в SoryCode.

1. Вы входите в **<a href={console}>SoryCode Zen</a>**, подписываетесь на Go и копируете свой API ключ.
2. Вы запускаете команду `/connect` в TUI, выбираете `SoryCode Go` и вставляете свой API ключ.
3. Запустите `/models` в TUI, чтобы увидеть список моделей, доступных через Go.

:::note
Только один участник рабочей области может подписаться на SoryCode Go.
:::
...
SoryCode Go включает следующие лимиты:
...
SoryCode Go — это план подписки за **$10/месяц**. Ниже приведены цены **за 1 млн токенов**.
...
[Model id](/docs/config/#models) в вашей конфигурации SoryCode использует формат `sorycode-go/<model-id>`. Например, для Kimi K2.5 вы бы использовали `sorycode-go/kimi-k2.5` в вашей конфигурации.
...
4. Не иметь **привязки к поставщику** (no lock-in), позволяя вам использовать любого другого провайдера с SoryCode.