Чтобы устранить проблемы с sorycode, начните с проверки журналов и локальных данных, которые он хранит на диске.
...
- **Windows**: нажмите `WIN+R` и вставьте `%USERPROFILE%\.local\share\sorycode\log`.
...
Вы можете установить уровень журнала с помощью CLI-параметра `--log-level`, чтобы получить более подробную информацию об отладке. Например, `sorycode --log-level DEBUG`.

---

## Хранилище

sorycode хранит данные сеанса и другие данные приложения на диске по адресу:

- **macOS/Linux**: `~/.local/share/sorycode/`
- **Windows**: нажмите `WIN+R` и вставьте `%USERPROFILE%\.local\share\sorycode`.
...
sorycode Desktop запускает локальный сервер sorycode (спутник `sorycode-cli`) в фоновом режиме. Большинство проблем вызвано неправильно работающим плагином, поврежденным кешем или неверными настройками сервера.
...
- Только для macOS: меню `SoryCode` -> **Обновить веб-просмотр** (помогает, если пользовательский интерфейс пуст или завис).
...
- **macOS/Linux**: `~/.config/sorycode/sorycode.jsonc` (или `~/.config/sorycode/sorycode.json`)
- **macOS/Linux** (более ранние версии): `~/.local/share/sorycode/sorycode.jsonc`
- **Windows**: нажмите `WIN+R` и вставьте `%USERPROFILE%\.config\sorycode\sorycode.jsonc`.
...
sorycode также может загружать локальные плагины с диска. Временно переместите их в сторону (или переименуйте папку) и перезапустите настольное приложение:

- **Глобальные плагины**
  - **macOS/Linux**: `~/.config/sorycode/plugins/`
  - **Windows**: нажмите `WIN+R` и вставьте `%USERPROFILE%\.config\sorycode\plugins`.
...
Если отключение плагинов не помогает (или установка плагина зависла), очистите кеш, чтобы sorycode мог его пересобрать.

1. Полностью закройте sorycode Desktop.
2. Удалите каталог кэша:

- **macOS**: Finder -> `Cmd+Shift+G` -> вставить `~/.cache/sorycode`.
- **Linux**: удалите `~/.cache/sorycode` (или запустите `rm -rf ~/.cache/sorycode`).
- **Windows**: нажмите `WIN+R` и вставьте `%USERPROFILE%\.cache\sorycode`.

3. Перезапустите рабочий стол sorycode.

---

### Исправить проблемы с подключением к серверу

sorycode Desktop может либо запустить собственный локальный сервер (по умолчанию), либо подключиться к настроенному вами URL-адресу сервера.
...
Если ваш `sorycode.json(c)` содержит раздел `server`, временно удалите его и перезапустите настольное приложение.

#### Проверьте переменные среды

Если в вашей среде установлен `SORYCODE_PORT`, настольное приложение попытается использовать этот порт для локального сервера.

- Отмените настройку `SORYCODE_PORT` (или выберите свободный порт) и перезапустите.
...
В Windows для sorycode Desktop требуется Microsoft Edge **WebView2 Runtime**. Если приложение открывается в пустом окне или не запускается, установите/обновите WebView2 и повторите попытку.

---

### Windows: общие проблемы с производительностью

Если вы испытываете низкую производительность, проблемы с доступом к файлам или проблемы с terminal в Windows, попробуйте использовать [WSL (подсистема Windows для Linux)](/docs/windows-wsl). WSL предоставляет среду Linux, которая более эффективно работает с функциями sorycode.

---

### Уведомления не отображаются

sorycode Desktop отображает системные уведомления только в следующих случаях:

- уведомления для sorycode включены в настройках вашей ОС, и
...
1. Закройте рабочий стол sorycode.
2. Найдите и удалите эти файлы (они находятся в каталоге данных приложения sorycode Desktop):

- `sorycode.settings.dat` (URL-адрес сервера по умолчанию для рабочего стола)
- `sorycode.global.dat` и `sorycode.workspace.*.dat` (состояние пользовательского интерфейса, например, недавние серверы/проекты)
...
Если у вас возникли проблемы с sorycode:
...
Чтобы выяснить, к каким моделям у вас есть доступ, запустите `sorycode models`.
...
2. Если проблема не устранена, попробуйте очистить сохраненную конфигурацию:
   ...
   В Windows нажмите `WIN+R` и удалите: `%USERPROFILE%\.local\share\sorycode`.
...
Если вы столкнулись с ошибками вызова API, это может быть связано с устаревшими пакетами провайдера. sorycode динамически устанавливает пакеты провайдеров (OpenAI, Anthropic, Google и т. д.) по мере необходимости и кэширует их локально.
...
1. Очистите кеш пакетов провайдера:
   ...
   В Windows нажмите `WIN+R` и удалите: `%USERPROFILE%\.cache\sorycode`.

2. Перезапустите sorycode, чтобы переустановить последние пакеты поставщиков.

Это заставит sorycode загружать самые последние версии пакетов провайдеров, что часто решает проблемы совместимости с параметрами модели и изменениями API.
...
sorycode определит, используете ли вы Wayland и предпочитаете `wl-clipboard`, в противном случае он попытается найти инструменты буфера обмена в порядке: `xclip` и `xsel`.