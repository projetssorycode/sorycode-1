Инструменты позволяют LLM выполнять действия в вашей кодовой базе. sorycode поставляется с набором встроенных инструментов, но вы можете расширить его с помощью [пользовательских инструментов](/docs/custom-tools) или [MCP-серверов](/docs/mcp-servers).
...
Вот все встроенные инструменты, доступные в sorycode.
...
:::note
Этот инструмент доступен только при `SORYCODE_EXPERIMENTAL_LSP_TOOL=true` (или `SORYCODE_EXPERIMENTAL=true`).
:::
...
:::note
Этот инструмент доступен только при использовании поставщика sorycode или когда для переменной среды `SORYCODE_ENABLE_EXA` установлено любое истинное значение (например, `true` или `1`).

Чтобы включить при запуске sorycode:
