...
Создайте `.sorycode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию sorycode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем sorycode [config](/docs/config):
...
- Глобальный: `~/.config/sorycode/commands/`
- Для каждого проекта: `.sorycode/commands/`
...
sorycode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
