...
SoryCode Go is a low cost **$10/month** subscription that gives you reliable access to popular open coding models.

:::note
SoryCode Go is currently in beta.
:::

Go works like any other provider in SoryCode. You subscribe to SoryCode Go and
...
SoryCode Go gives you access to these models for **$10/month**.

---

## How it works

SoryCode Go works like any other provider in SoryCode.

1. You sign in to **<a href={console}>SoryCode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `SoryCode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to SoryCode Go.
:::
...
SoryCode Go includes the following limits:
...
uses the format `sorycode-go/<model-id>`. For example, for Kimi K2.5, you would
use `sorycode-go/kimi-k2.5` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with SoryCode as well.